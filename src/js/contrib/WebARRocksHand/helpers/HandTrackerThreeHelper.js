...
const HandTrackerThreeHelper = (function () {
  ...
  let _spec = null,
    _landmarksStabilizerModule = null;
  let _landmarksStabilizers = null,
    _poseFilters = null,
    _neuralNetworkIndices = null;
  ...
  function compile_shader(source, glType, typeString) {
    ...
    if (!_gl.getShaderParameter(glShader, _gl.COMPILE_STATUS)) {
      alert(
        'ERROR IN ' + typeString + ' SHADER: ' + _gl.getShaderInfoLog(glShader)
      );
  ...
  function init_shps() {
    ...
    _shps.drawVideo.uniforms.transformMat2 = _gl.getUniformLocation(
  ...
  function init_debugDisplayLandmarks() {
    ...
    _debugDisplayLandmarks.vertices = new Float32Array(
      _debugDisplayLandmarks.LMLabels.length * 2
    );
    ...
    _gl.bufferData(
      _gl.ARRAY_BUFFER,
      _debugDisplayLandmarks.vertices,
      _gl.DYNAMIC_DRAW
    );
    ...
    _gl.bindBuffer(
      ...
      _debugDisplayLandmarks.glIndicesVBO
    );
  ...
  function init_poseEstimation() {
    ...
    _poseEstimation.poseLandmarksIndices = _spec.poseLandmarksLabels.map(
      function (label) {
    ...
    // init imgPoints:
    _poseEstimation.imgPointsPx = _poseEstimation.poseLandmarksIndices.map(
      function () {
        return [0, 0];
      }
    );
  ...
  function draw_video() {
    ...
    _gl.uniformMatrix2fv(
      _shps.drawVideo.uniforms.transformMat2,
      false,
      _videoTransformMat2
    );
  ...
  function draw_landmarks(detectState) {
    ...
    _gl.drawElements(
      _gl.POINTS,
      _debugDisplayLandmarks.LMLabels.length,
      _gl.UNSIGNED_SHORT,
      0
    );
  }
  ...
  function callbackTrack(detectStatesArg) {
    ...
    const detectStates =
      _spec.maxHandsDetected === 1 ? [detectStatesArg] : detectStatesArg;
    ...
    for (let i = 0; i < detectStates.length; ++i) {
      ...
      if (detectState.isDetected) {
        ...
        if (_spec.debugLogHandInfo) {
          console.log(
            'isRightHand: ',
            detectState.isRightHand,
            'isFlipped: ',
            detectState.isFlipped
          );
        }
  ...
  function compute_pose(landmarks, isRightHand, poseFilter, handIndex) {
    ...
    const objPoints = isRightHand
      ? _poseEstimation.objPointsRight
      : _poseEstimation.objPointsLeft;

    // compute pose:
    const focals = _poseEstimation.focals;
    const solved = WEBARROCKSHAND.compute_pose(
      objPoints.points,
      imgPointsPx,
      focals[0],
      focals[1]
    ...
    const tracker =
      isRightHand || !_spec.enableFlipObject
        ? _three.trackersRight[handIndex]
        : _three.trackersLeft[handIndex];
    tracker.visible = true;
    tracker.position.copy(objPoints.mean).multiplyScalar(-1);

    // hide the other side:
    const trackerHidden =
      tracker === _three.trackersRight[handIndex]
        ? _three.trackersLeft[handIndex]
        : _three.trackersRight[handIndex];
    if (trackerHidden) {
    ...
    if (poseFilter !== null) {
      filteredMovMatrix = poseFilter.update(
        imgPointsPx,
        _three.trackersParent[handIndex],
        _three.camera,
        that.get_viewHeight(),
  ...
  function init_landmarksStabilizers(landmarksStabilizerSpec) {
    ...
    for (let i = 0; i < _spec.maxHandsDetected; ++i) {
      stabilizers.push(
        _landmarksStabilizerModule.instance(landmarksStabilizerSpec)
      );
    }
    return stabilizers;
  }
  ...
  const that = {
    init: function (spec, stabilizerModule) {
      ...
      _landmarksStabilizers = init_landmarksStabilizers(
        _spec.landmarksStabilizerSpec,
        _landmarksStabilizerModule
      );
    ...
    update: function (specUpdated) {
      ...
      return WEBARROCKSHAND.update({
        ...
        if (typeof specUpdated.landmarksStabilizerSpec !== 'undefined') {
          _landmarksStabilizers = init_landmarksStabilizers(
            specUpdated.landmarksStabilizerSpec
          );
        }
    ...
    set_handRightFollower: function (threeObjectParent0, threeObject0) {
      ...
      for (let i = 0; i < _spec.maxHandsDetected; ++i) {
        ...
        threeObjectParent0.traverse(function (threeStuff) {
          isContainsSkinnedMesh =
            isContainsSkinnedMesh || threeStuff.isSkinnedMesh;
        });
        if (_spec.maxHandsDetected > 1 && isContainsSkinnedMesh) {
          throw new Error(
            'Multihands is not supported by the helper with skinned mesh.'
          );
        }
        //isContainsSkinnedMesh = isContainsSkinnedMesh || (i===0);
        const threeObjectParent =
          isContainsSkinnedMesh || !_spec.enableFlipObject
            ? threeObjectParent0
            : threeObjectParent0.clone();
        ...
        threeObject.traverse(function (threeStuff) {
          ...
          if (ud.isSoftOccluder) {
            const softOccluder = extract_threeSoftOccluder(
              threeStuff,
              ud.softOccluderRadius,
              ud.softOccluderDr,
              false
            );
    ...
    update_threeCamera: function (sizing, threeCamera) {
      ...
      fov = Math.min(
        Math.max(fov, _spec.cameraFovRange[0]),
        _spec.cameraFovRange[1]
      );
      ...
      if (canvasAspectRatio > videoAspectRatio) {
        ...
        const cvhs = vh * scale;
        fov =
          (2 * Math.atan((cvh / cvhs) * Math.tan(0.5 * fov * _deg2rad))) /
          _deg2rad;
      }
      console.log(
        'INFO in update_threeCamera(): camera vertical estimated FoV is',
        fov,
        'deg'
      );
