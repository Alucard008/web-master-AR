...
const HandTrackerNavigationHelper = (function () {
  ...
  function build_shaderProgram(
    ...
    const glShaderVertex = compile_shader(
      gl,
      GLSLprecision + shaderVertexSource,
      gl.VERTEX_SHADER,
      'VERTEX ' + id
    );
  ...
  function init_htNav() {
    ...
    _shps.video = build_shaderProgram(
      _gl,
    ...
    _shps.video.uniforms.transformMat2 = _gl.getUniformLocation(
      _shps.video.program,
      'transform'
    );

    // build shaderprograms for pointer rendering:
    _shps.pointerPoints = build_shaderProgram(
      _glp,
      'attribute vec2 position;\n\
    ...
    _shps.pointerLines = build_shaderProgram(
      _glp,
      'attribute vec2 position;\n\
      attribute float isPointer;\n\
      varying float vIsPointer;\n\
    ...
    _shps.pointerCursor = build_shaderProgram(
      _glp,
      'attribute vec2 position;\n\
  ...
  function draw_video() {
    ...
    _gl.uniformMatrix2fv(
      _shps.video.uniforms.transformMat2,
      false,
      _videoTransformMat2
    );
  ...
  function draw_pointer(landmarks) {
    ...
    _glp.bindBuffer(_glp.ARRAY_BUFFER, _glVBOs.lmPositions);
    _glp.bufferData(_glp.ARRAY_BUFFER, _landmarks.positions, _glp.DYNAMIC_DRAW);
    _glp.vertexAttribPointer(
      _shps.pointerLines.attributes.position,
      2,
      _glp.FLOAT,
      false,
      8,
    ...
    _glp.bindBuffer(_glp.ARRAY_BUFFER, _glVBOs.lmIsPointer);
    _glp.vertexAttribPointer(
      _shps.pointerLines.attributes.isPointer,
      1,
      _glp.FLOAT,
      false,
      4,
    ...
    _glp.uniform1f(
      _shps.pointerCursor.uniforms.downFactor,
      _pointer.downFactor
    );
    _glp.uniform2f(
      _shps.pointerCursor.uniforms.cursorPosition,
      _pointer.x,
      _pointer.y
    );
    _glp.uniform2f(
      _shps.pointerCursor.uniforms.cursorScale,
      (2 * _spec.cursorSizePx * _pointer.mirrorXFactor) / _dims.width,
      (2 * _spec.cursorSizePx) / _dims.height
    );
    _glp.bindBuffer(_glp.ARRAY_BUFFER, _glVBOs.lmCursorPositions);
  ...
  function update_click(isFlipped, landmarks, handWidth) {
    ...
    const pointerDistances = isFlipped
      ? _spec.pointerDistancesBackSide
      : _spec.pointerDistancesPalmSide;
    ...
    if (isDown) {
      ...
      _pointer.downFactor = Math.max(
        0,
        1.0 - Math.abs(dl - dlRef) / _spec.pointerHeatDistance
      );
    }

    // save result:
    (_pointer.xPx = xPx), (_pointer.yPx = yPx);
    (_pointer.x = (2 * xMean) / _dims.width),
      (_pointer.y = (2 * yMean) / _dims.height);
  ...
  function size_canvas(canvas) {
    ...
    (_dims.width = window.innerWidth + _dims.marginRight + _dims.marginLeft),
      (_dims.height =
        window.innerHeight + _dims.marginTop + _dims.marginBottom);
    [_spec.canvasVideo, _spec.canvasPointer].forEach(function (canvas) {
      (canvas.width = _dims.width), (canvas.height = _dims.height);
  ...
  const that = {
    init: function (spec) {
      ...
      _pointer.blendRange = [
        _spec.pointerBlendHandRadiusRange[0] * _spec.cursorSizePx,
        _spec.pointerBlendHandRadiusRange[1] * _spec.cursorSizePx,
      ];
    ...
    start: function () {
      ...
      WEBARROCKSHAND.init({
        ...
        callbackReady: function (err, objs) {
          ...
          _landmarks.pointerIndices = _spec.pointerLandmarks.map(function (
            lmLabel
          ) {
            return _landmarks.allLabels.indexOf(lmLabel);
          });
    ...
    destroy: function () {
      ...
      _isSelfieCam = false;
      return WEBARROCKSHAND.destroy();
    },
  }; //end that
  return that;
})();

export default HandTrackerNavigationHelper;
